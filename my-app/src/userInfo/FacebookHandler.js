...
const FacebookHandler = ({ clientId, user }) => {
  const [showInput, setShowInput] = useState(false); // Para mostrar/ocultar el formulario de entrada
  
  const loginInsta = (event) => {
    const authUrl = `https://zgo5ag3batxfe3pclxgsksdfly0jczub.lambda-url.us-east-1.on.aws/init-auth/8?clientId=${clientId}&redirectUrl=https://simple-ai-alpha.vercel.app/Perfil`;
